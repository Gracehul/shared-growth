...
myCobot 280 JN to reliably draw simple shapes on paper via `bao_arm`. No camera tracking,
human-robot interaction, or growth-algorithm logic yet — out of scope for this milestone.

## Environment

- No `pyproject.toml`/`requirements.txt` in this repo. Everything runs against the venv at
  `/home/tomas/.venv-py/general/bin/python3`, which already has `pymycobot` and `numpy`
  installed.
...
```
python3 -m py_compile bao_arm/*.py scripts/*.py                # syntax check
python3 -c "from bao_arm import Arm, config, ik, kinematics"   # import check
...
touching `bao_arm/` before considering a change done.

## Architecture

### `bao_arm/` is a vendored, trimmed clone of the thesis `armik` library — not a dependency

`armik` (at `/home/tomas/University/Thesis/MyCobot_Pkg/Thesis_python_MyCobot_pkg/thesis_armik_pkg/armik/`)
is a separate, already-hardware-validated library from an unrelated thesis project. `bao_arm`
...
extending `bao_arm`, port the corresponding `armik` source as a reference rather than
...
firmware to do FK/IK over the serial link, which was found unreliable in testing. `bao_arm`
...
the main reason `bao_arm` exists instead of calling `pymycobot` directly.

### Unit boundary: millimetres/degrees internally, centimetres/degrees at the public API

Everything inside `bao_arm` (DH table, kinematics, IK) works in mm/deg. The mm↔cm conversion
...
`bao_arm`. They import `bao_arm.robot`/`bao_arm.shapes`, which no longer exist after the pivot
...
`scripts/` holds current, working scripts against the new `bao_arm.Arm` API
...
### Calibration constants in `bao_arm/config.py`
